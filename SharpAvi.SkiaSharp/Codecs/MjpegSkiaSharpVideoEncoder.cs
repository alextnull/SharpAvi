...
namespace SharpAvi.Codecs
{
    ...
    /// </summary>
    /// <remarks>
    /// The implementation relies on Encoder from the <c>SkiaSharp</c> package.
    /// </remarks>
    public sealed class MJpegSkiaSharpVideoEncoder : IVideoEncoder
    {
        ...
        /// </param>
        public MJpegSkiaSharpVideoEncoder(int width, int height, int quality)
        {
        ...
        private int LoadAndEncodeImage(ReadOnlySpan<byte> source, Stream destination)
        {
            ...
            var skInfo = new SKImageInfo(width, height, SKColorType.Bgra8888);
            using (var skImage = SKImage.FromPixelCopy(skInfo, source))
            using (var skData = skImage.Encode(SKEncodedImageFormat.Jpeg, quality))
            {
                skData.SaveTo(destination);
            }
            destination.Flush();
